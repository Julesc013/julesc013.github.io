...
| `C2` | Prepare parity-safe source/output wiring and migration steps while preserving the canonical content path at `/`. |
| `D1` | Normalize and structure canonical content for the source/build layer without changing user-facing meaning. |
...
- Development branches are source-only: `src/` is authoritative, `_site/` is generated, and duplicate root-authored site files are retired once `npm run build:parity` passes.
- The Pages workflow builds `_site/` from `main`; development branches remain non-live until merged or otherwise promoted.

## Manual-Safe Default
- If the requested prompt group is ambiguous, fall back to audit, documentation, or narrowly scoped fixes.
- Do not assume permission to restructure the repository, change deployment policy, or expand shell scope just because later groups exist.
- When repository state and prompt intent disagree, prefer preserving the current user-visible behavior and document the conflict.
- If a governance document blocks safe implementation, complete the governance work before writing code.
...
- `C` and `D` groups verify directory-model correctness, source/output separation, and unchanged user-facing behavior.
...
- Preserve the canonical content path at `/`.
...
- Do not reactivate duplicate root-authored site files on development branches once source-driven parity is passing.
